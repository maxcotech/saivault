...
class FileManagerController extends Controller with FileExtension,PathMixin,EncryptionMixin{
  ...
  Future<bool> entityHasCryptFile(cryptPath) async {
    ...
    switch(type){
      case FileSystemEntityType.directory: return true;
      case FileSystemEntityType.file: return true;
      case FileSystemEntityType.notFound: return false;
      default: return false;
    }
  }
  Future<void> onDeleteTrackedItem(HiddenFileModel model)async{
    try{
      if(model.hidden == 1 && await this.entityHasCryptFile(model.hiddenPath)){
  ...
  Future<bool> hideEntity(String originalPath,String cryptPath,e.Key key,String ivString,bool isNested)async{
      ...
      if(model.hidden == 0){
        ...
        try{
          if(await this.hideEntity(model.originalPath,model.hiddenPath,
            ...
            throw new Exception('Failed to hide entity, please verify that the file system entity you wish to hide actually exists.');
          }
        }
  ...
  Future<bool> restoreEntity(String originalPath,String cryptPath,e.Key key,String ivString,bool isNested)async{
  ...
  Future<bool> restoreFile(String sourcePath,String desPath,e.Key key,String ivString)async{
  ...
  Future<bool> hideFile(String sourcePath,String desPath,e.Key key,String ivString,bool isNested)async{
    var channelService = StorageChannelService();
    if(await channelService.isStoragePermissionGranted()){
      return await this.hideFileExecutor(sourcePath,desPath,key,ivString,isNested);
    } else {
      if(await confirmDialog(message:'Please choose the root directory of your external storage (e.g usdcard1) on the following screen to grant $APPNAME write permission')){
        if(await channelService.requestStorageAccess(await this.getStoragePathByEntity(sourcePath))){
          return false;
        }
        return false;
      }
  ...
  Future<bool> hideFileExecutor(String sourcePath,String desPath,e.Key key,String ivString,bool isNested)async{
    ...
    try{
      if(hiddenEntities.length > 0){
        ...
        if(_hiddenFiles.length > 0) {
          Get.rawSnackbar(message:'All entities has been restored.',duration:Duration(seconds:3));

        } else {
          Get.rawSnackbar(message:'Nothing to restore',duration:Duration(seconds:3));
        }
      }
