...
class DriveServices {
  ...
  Future<DriveServices> init() async {
    ...
    if (this.account != null) {
      ...
      Get.rawSnackbar(
          message: "Google Sign in failed.", duration: Duration(seconds: 2));
    }
  ...
  Future<void> signInToDrive() async {
    ...
    final googleSignIn = GoogleSignIn.standard(
        scopes: [DriveApi.driveScope, DriveApi.driveAppdataScope]);
  ...
  Future<List<File>> getBackedUpFiles() async {
    if (this.driveApi == null)
      throw new Exception('Backup Service was not properly initiialized.');
    FileList fileList =
        await this.driveApi.files.list(spaces: this.driveFolder);
    for (var file in fileList.files) {
      print(
          "file name: ${file.name} , file original name: ${file.originalFilename} file id: ${file.id}");
    }
  ...
  Future<io.File> downloadDatabaseFile(String fileId) async {
    ...
    if (this.driveApi == null)
      throw new Exception('Backup Service was not properly initialized.');
    Media file = await driveApi.files
  ...
  Future<String> downloadJsonById(String fileId) async {
    ...
    await for (var data in file.stream){
      dataString.insertAll(dataString.length, data);
    }
    return utf8.decode(dataString);
  }
  ...
  Future uploadJsonToDrive(String str, String driveFileName) async {
    if (this.driveApi == null)
      throw new Exception('Backup service was not properly initialized.');
    if (str != null && str.isNotEmpty) {
      List<int> dataList = utf8.encode(str);
      Media media =
          new Media(Future.value(dataList).asStream(), dataList.length, contentType:'application/json');
      var driveFile = new File();
      driveFile.name = driveFileName;
      var fileId = await this.getIdOfNamedFile(driveFileName);
      var result;
      if (fileId == null) {
        driveFile.parents = [this.driveFolder];
        result =
            await this.driveApi.files.create(driveFile, uploadMedia: media);
      } else {
        result = await this
            .driveApi
            .files
  ...
  Future uploadFileToDrive(io.File file) async {
    if (file == null || await file.exists() == false)
      throw new Exception('File to backup is invalid.');
    if (this.driveApi == null)
      throw new Exception('Backup service was not properly initialized.');
    ...
    var fileId = await this.getIdOfNamedFile(fileName);
    var result;
    if (fileId == null) {
      driveFile.parents = [this.driveFolder];
